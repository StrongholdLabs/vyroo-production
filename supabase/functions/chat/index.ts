    ...
    const outputStream = new ReadableStream({
      async start(controller) {
        try {
          ...
          // Generate follow-up suggestions — try AI first, fallback to context-aware defaults
          {
            let followUpItems: Array<{text: string; category: string}> = [];

            // Try AI-generated follow-ups with a tight timeout
            ...
            // Always ensure we have follow-ups — use smart fallback based on content
            if (followUpItems.length === 0) {
              const hasReport = !!lastReportContent;
              followUpItems = hasReport ? [
                { text: "Dive deeper into the top findings", category: "research" },
                { text: "Create a comparison table", category: "analysis" },
                { text: "What are the key takeaways?", category: "research" },
                { text: "Summarize this for a presentation", category: "create" },
              ] : [
                { text: "Tell me more about this", category: "research" },
                { text: "Can you give specific examples?", category: "research" },
